...
public class PdfNameTree {
    ...
    public static PdfDictionary writeTree(HashMap items, PdfWriter writer) throws IOException {
        ...
        if (names.length <= leafSize) {
            ...
            for (int k = 0; k < names.length; ++k) {
                ar.add(new PdfString(names[k], PdfObject.TEXT_UNICODE ));
	    ...
	    arr.add(new PdfString(names[offset], PdfObject.TEXT_UNICODE));
	    arr.add(new PdfString(names[end - 1], PdfObject.TEXT_UNICODE));
        ...
        while (true) {
            ...
            for (int k = 0; k < tt; ++k) {
                ...
                PdfDictionary dic = new PdfDictionary();
                PdfArray arr = new PdfArray();
		arr.add(new PdfString(names[k * skip], PdfObject.TEXT_UNICODE));
		arr.add(new PdfString(names[Math.min((k + 1) * skip, names.length) - 1], PdfObject.TEXT_UNICODE));
                dic.put(PdfName.LIMITS, arr);
                arr = new PdfArray();
                for (; offset < end; ++offset) {
                    arr.add(kids[offset]);
    ...
    private static void iterateItems(PdfDictionary dic, HashMap items) {
		...
		items.put(s.toUnicodeString(), arr.get(k));
            }
        }
